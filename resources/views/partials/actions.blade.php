<div class="d-grid gap-2 mt-3">
    @if ($appt->status === \App\Enums\AppointmentStatus::Upcoming)
        <div class="d-flex flex-wrap gap-2 justify-content-center">
            ...
            @endif
            <form action="{{ route('antrian.cancel', $appt) }}" method="POST">
                @csrf
                <button class="btn btn-sm btn-outline-danger flex-grow-1">
                    Batalkan
                </button>
            </form>
            <a class="btn btn-sm btn-outline-primary flex-grow-1 tiket-reschedule-btn"
                data-modal-url="{{ route('antrian.reschedule', $appt) }}" data-modal-title="Jadwal Ulang Tiket">
                Jadwal&nbsp;Ulang
            </a>
